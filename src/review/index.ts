...
export const review = async (yargs: ReviewArgs): Promise<void> => {
  ...
  logger.debug(`WorkSpace: ${yargs.workSpace}`)
  if (yargs.baseUrl) {
    logger.debug(`Base URL: ${yargs.baseUrl}`);
  }

  const files: ReviewFile[] = await getFilesWithChanges(yargs.platform, yargs.workSpace);
  ...
  const platformProvider = await getPlatformProvider(yargs.platform, yargs.workSpace);
  logger.debug('Platform provider:', platformProvider);

  const model = createModel(yargs.modelString, { baseURL: yargs.baseUrl });
  const prompt = await constructPrompt(filteredFiles, yargs.reviewLanguage, yargs.workSpace);
  logger.debug('Prompt:', prompt);

  try {
    const message = await runAgenticReview(prompt, model, platformProvider, yargs.maxSteps, yargs.workSpace);
