...
import {getGitRoot, getGitRootWorkSpace} from '../../git/getChangedFilesNames';
...
export const localProvider = async (workSpace: string | undefined): Promise<PlatformProvider> => {
  ...
  logger.debug("reviewFilePath", reviewFilePath)
  const shippieDirPath = path.dirname(reviewFilePath);
  logger.debug("shippieDirPath", shippieDirPath)
